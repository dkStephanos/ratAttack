    ...
    class RatAttack
    {
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < numScenarios; i++)
            {
                ...
                grids[i] = new Grid(numRatPopulations, bombStrengths[i]);
                ...
                //Clears blank line between ratPopulation data
                Console.ReadLine();
            }

            for (int j = 0; j < numScenarios; j++)
            {
                //Sort the rat populations
                Array.Sort(grids[j].ratPopulations);

                //Run the algorithm to determine the bomb location, and display results to screen
                Console.WriteLine(grids[j].plantBomb());
            }
